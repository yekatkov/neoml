...
inline void CBlobConvolution<32>::CCode::fillSingleProcessingKernel( CBlobConvolution<32>& bc, bool useNarrowProcessing, int windowIndex )
{
	...
	initProcessingMainLoop( bc, &res[0], stepCount, stepSize, labelProcessingKernel, labelFillProcessingKernelEnd,  windowIndex );
...
//template<>
//inline void CBlobConvolution<64>::batchProcessChannels( const float* srcPtr, const float* fltPtr,
...
//template<>
//inline void CBlobConvolution<64>::singleProcessChannels( const float* srcPtr, const float* fltPtr,
...
//template<>
//inline void CBlobConvolution<64>::batchProcess( const float* srcPtr, float* resPtr, size_t windowIndex, bool /*useNarrowProcessing*/ )
//{
//	const float* tempFreeTerm = freeTerm;
//	const float* tempFlt = flt;

//	for( int i = 0; i < 2; i++ ) {
//		const __m256 ft0 = freeTerm != nullptr ? _mm256_loadu_ps( tempFreeTerm ) : _mm256_setzero_ps();
//		const __m256 ft1 = freeTerm != nullptr ? _mm256_loadu_ps( tempFreeTerm + 8 ) : _mm256_setzero_ps();
//		const __m256 ft2 = freeTerm != nullptr ? _mm256_loadu_ps( tempFreeTerm + 16 ) : _mm256_setzero_ps();
//		const __m256 ft3 = freeTerm != nullptr ? _mm256_loadu_ps( tempFreeTerm + 24 ) : _mm256_setzero_ps();
...
//			batchProcessChannels( srcPtr + *srcIt, tempFlt + *fltIt, r00, r01, r02, r03, r10, r11, r12, r13 );
//		}
...
//		tempFreeTerm += 32;
//		resPtr += 32;
//		tempFlt += FltW * FltH * ChCnt * 32;
//	}
//}

//template<>
//inline void CBlobConvolution<64>::singleProcess( const float* srcPtr, float* resPtr, size_t windowIndex, size_t /*offset*/ )
//{
//	const float* tempFreeTerm = freeTerm;
//	const float* tempFlt = flt;

//	for( int i = 0; i < 2; i++ ) {
//		__m256 r0 = freeTerm != nullptr ? _mm256_loadu_ps( tempFreeTerm ) : _mm256_setzero_ps();
//		__m256 r1 = freeTerm != nullptr ? _mm256_loadu_ps( tempFreeTerm + 8 ) : _mm256_setzero_ps();
//		__m256 r2 = freeTerm != nullptr ? _mm256_loadu_ps( tempFreeTerm + 16 ) : _mm256_setzero_ps();
//		__m256 r3 = freeTerm != nullptr ? _mm256_loadu_ps( tempFreeTerm + 24 ) : _mm256_setzero_ps();

//		auto srcIt = SrcPixelsOffset[windowIndex].cbegin();
//		auto fltIt = FltPixelsOffset[windowIndex].cbegin();
//		for( ; srcIt != SrcPixelsOffset[windowIndex].cend(); srcIt++, fltIt++ ) {
//			singleProcessChannels( srcPtr + *srcIt, tempFlt + *fltIt, r0, r1, r2, r3 );
//		}
